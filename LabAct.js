...
function startCountdown() {
    const countdownInterval = setInterval(function () {
        countdownTime--;
        timerElem.textContent = `Time remaining: ${countdownTime} seconds`;

        if (countdownTime <= 0) {
            clearInterval(countdownInterval);
            timerElem.textContent = "Time's up! You can't add a team anymore.";
            teamInputElem.disabled = true;
        } else {
            teamInputElem.disabled = false;  
        }
    }, 1000);  
}
...
teamInputElem.addEventListener("keyup", function (e){
    if(e.key ==='Enter'){
        ...
        if(nbaTeams.won.hasOwnProperty(team)){
            console.log("Yes won nba titles");
